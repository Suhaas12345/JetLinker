...
vector<int> dijkstra(vector<City>& cities, int start, int end, vector<int>& predecessors) {
    ...
    for (int i = 0; i < numCities - 1; i++) {
        ...
        // Update the distances and predecessors of the neighboring cities
...
int main() {
    ...
    vector<int> predecessors(numCities, -1);
    vector<int> shortestPathDijkstra = dijkstra(cities, start - 1, end - 1, predecessors);
    ...
    if (shortestPathDijkstra[end - 1] == INT_MAX) {
        ...
        cout << shortestPathDijkstra[end - 1] << " minutes" << endl;

        // Print the cities in the shortest path
        cout << "Cities in the shortest path: ";
        int current = end - 1;
        while (current != -1) {
